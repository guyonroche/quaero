...
function xform(row, copy, change) {
  let value = {};
  copy.forEach(name => value[name] = row[name]);
  Object.keys(change).forEach(name => value[name] = row[change[name]]);
  return value;
}
...
class MySqlDAL {
  ...
  // ==============================================================================
  // Questions
  createQuestion(quid, created, user_id, title, tags, text) {
    let sql = 'INSERT into question SET ?;';
    return this.execute(sql, {quid, created, user_id, title, tags, text})
      .then(data => data.insertId);
  }

  getQuestionByQuid(quid) {
    let sql = 'SELECT id, quid, created, user_id, title, tags, text FROM question WHERE quid = ?';
